...
main_application.title('Text Editor By Neeraj')
...
def change_font(main_application):
    ...
    curren_font_family = font_family.get()
...
def changed(event = None):
    ...
    if text_editor.edit_modified():
        ...
        status_bar.config(text = 'Characters : {}, Words : {}'.format(characters, words))
...
def save_file(event = None):
    ...
    try:
        if url:
            ...
            url = filedialog.asksaveasfile(mode = 'w', defaultexension = '.txt', filetypes = (('Text Files', '*.txt'), ('All Files', '*.*')))
...
def save_as(event = None):
    global url
    try:
        conetent = text_editor.get(1.0, tk.END)
        url = filedialog.asksaveasfile(mode = 'w', defaultexension = '.txt', filetypes = (('Text Files', '*.txt'), ('All Files', '*.*')))
        url.write(conetent)
        url.close()
    except:
...
edit.add_command(label = 'Paste', image = paste_icon, compound = tk.LEFT, accelerator = 'Ctrl+V', command = lambda : text_editor.event_generate("<Control x>"))
...
def find_func(event = None):
    ...
    # button grid
    find_btn.grid(row = 2, column = 0, padx = 10, pady = 5)
    replace_btn.grid(row = 2, column = 1, padx = 10, pady = 5)
...
view.add_checkbutton(label = 'Toolbar', onvalue = True, offvalue = False , variable = show_toolbar, image = tool_bar_icon, compound = tk.LEFT, command = hide_toolbar)
